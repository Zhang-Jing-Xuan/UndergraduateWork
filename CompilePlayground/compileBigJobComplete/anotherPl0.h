...
/*虚拟机代码*/
enum fct{
	lit, opr,  lod,  sto,  cal,  inte,  jmp,  jpc, lda, sta  /* lda, sta 增加两个虚拟机指令lda,sta,分别用来从数组中取数和存到数组中 */
};
#define fctnum 10
/*虚拟机代码结构*/
struct instruction
...
#define arraycoefdo(a,b,c)					  if(-1==arraycoef(a,b,c)) return -1   /*增加数组声明的处理*/
void error(int n);
int getsym();
int getch();
void init();
