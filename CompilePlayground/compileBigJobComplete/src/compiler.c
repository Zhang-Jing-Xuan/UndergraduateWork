...
void error(long n) // 输出报错信息
{
  long i;

  printf(" ****");
  for (i = 1; i <= cc - 1; i++)
  { // character count, 刚刚读的字符的下标（非下一位置）
    printf(" ");
  }
  switch(n){
    case 1:
    ...
    case 45:
            printf("if/while的测试条件的类型应是\n");
            break;

    case 46:
            printf("只能输入输出整型或实型数据\n");
            break;
...
void getch()
{
  ...
  { // ll:line length
    ...
    ll = 0; /*将指针调到初始位置*/
    cc = 0; // 刚刚读的字符的下标（非下一位置）
    printf("%5d ", cx); // code allocation index，指向下一位置
    while ((!feof(infile)) && ((ch = (char)(getc(infile))) != '\n')) // ch:last character read
    {
      printf("%c", ch);
      ++ll; // 这一行的长度加一
      line[ll] = ch; // line:缓存一行代码
    }
    printf("\n");
    ++ll;
    line[ll] =
...
LABEL_INIT:
  ...
  { // identified or reserved
    assert(a[al] == '\0'); // al=10:length of identifiers
    k = 0; // k 记录标识符的长度
    do
    {
      if (k < al)
...
{
  if (cx > cxmax) // code allocation index，指向下一位置
  {
...
void test(SymbolType s1, SymbolType s2, long n)
{
  if (!(sym & s1))
  {
    error(n);
    s1 = s1 | s2;
    while (!(sym & s1)) /*当检测不通过时，不停获取符号，直到它属于需要的集合或补救的集合*/
    {
      getsym();
    }
  }
...
*/
long position(void *which_table, char *id)
{ // find identifier id in table
  ...
  if (which_table == (void *)table)
  {
    ...
    i = tx; //tx:current table index(非下一位置)
...
void constdeclaration()
{
  ...
  if (sym == ident)
  {
    ...
    if (sym == eql || sym == becomes)
    {
      ...
      if (sym == number)
      {
        enter(g_id, CONSTANT, sym_type);  /*把这个常量登陆到名字表*/
        getsym();
      }
      else
      {
  ...
  {
    error(4); // const, type, var, procedure, function 后应为标识符
  }
}
...
*/
DataType simp_exp(SymbolType fsys, DataType down_type) // 加减
{
  SymbolType addop = 0;
...
DataType expr(SymbolType fsys, DataType down_type) // 不等号
{
  SymbolType relop;
  DataType sym_type = NULL;
  ...
  if (sym & (eql | neq | lss | gtr | leq | geq))
  {
    ...
    case lss:
      ...
      {
        gen(opr, 0, 31); // convert integer(s[t-1]) to real
        gen(opr, 0, 26);
      }
      else if (expr_type1 == REAL && expr_type2 == INTEGER)
      {
        gen(opr, 0, 30); // convert integer(s[t]) to real
        gen(opr, 0, 26);
      }
      else
      {
  ...
  do
  {
    ...
    if (sym == varsym)
    {
      ...
      do
      {
        ...
        {
          error(37); // 是否应该补救？ 定义变量或函数时缺少冒号
        }
        if (sym & (integersym | realsym | booleansym |
                   ident))
        { // 应检查 ident 是否为数组类型 TODO:
          long i;
          for (i = tx1; i <= tx; ++i)
          {
            table[i].type =
