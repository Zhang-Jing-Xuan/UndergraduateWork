...
extern int g_nWordsIndex; //g_nWordsIndex是已识别单词的序号
...
int StatementParsing(int nIndentNum) //nIndentNum是打印时要缩进的空格数
{
	...
	case IF:											 //如果第一个单词是"if",按"条件语句"处理
		...
		{																   //"条件"前应有左括号
			...
			return ERROR;									   //缺少左括号
		if (g_Words[g_nWordsIndex].eType == RIGHT_PARENTHESIS) //"条件"后应有"右括号"
		{
			PrintInParsing(g_nWordsIndex, nIndentNum, "RIGHT_PARENTHESIS"); //打印调试信息
			g_nWordsIndex++;												//取下一个单词
			PrintInParsing(g_nWordsIndex, nIndentNum, "StatementParsing");	//打印调试信息
			nResult = StatementParsing(nIndentNum + 1);						//右括号后应是"语句"
			if (nResult != ERROR)
			{
				if (g_Words[g_nWordsIndex].eType == ELSE)
				{																   //"语句"之后若有ELSE，进入ELSE分语句
	...
	case NOT:											  //如果第一个单词是"not",按"条件语句"处理
		PrintInParsing(g_nWordsIndex, nIndentNum, "NOT"); //打印调试信息
		g_nWordsIndex++;
		if (g_Words[g_nWordsIndex].eType == LEFT_PARENTHESIS)
		{																   //"条件"前应有左括号
			PrintInParsing(g_nWordsIndex, nIndentNum, "LEFT_PARENTHESIS"); //打印调试信息
			g_nWordsIndex++;											   //取下一个单词
			PrintInParsing(g_nWordsIndex, nIndentNum, "ConditionParsing"); //打印调试信息
			ConditionParsing(nIndentNum + 1);							   //"NOT"后是"条件"Condition
		}
		else
			return ERROR;									   //缺少左括号
		if (g_Words[g_nWordsIndex].eType == RIGHT_PARENTHESIS) //"条件"后应有"右括号"
...
int ConditionParsing(int nIndentNum) //nIndentNum是打印时要缩进的空格数
{
	...
	else
	{
		...
		if (g_Words[g_nWordsIndex].eType == EQL //如果后面有逻辑运算符
			|| g_Words[g_nWordsIndex].eType == NEQ || g_Words[g_nWordsIndex].eType == LES || g_Words[g_nWordsIndex].eType == LEQ || g_Words[g_nWordsIndex].eType == GTR || g_Words[g_nWordsIndex].eType == GEQ)
		{
