...
bool isInItems(LR1Items &I, LR1Item &t)
{
    for (auto it = I.items.begin(); it != I.items.end(); it++)
    {
        LR1Item tmp = *it;
        if (tmp.next==t.next&&tmp.location == t.location && tmp.production == t.production)
            return true;
    }
    return false;
}

int isInC(LR1Items &I)
{
    ...
    while (change)
    {
        ...
        for (auto it = J.items.begin(); it != J.items.end(); it++)
        {
            ...
            if (t.location < t.production.right.size())
            {
                ...
                set<char>newNext;
                vector<char>B(t.production.right.begin()+t.location+1,t.production.right.end());
                B.push_back(t.next);
                getAlphaFirst(B,newNext);
                if (isNT(c))
                {
...
void go(LR1Items &I, char c, LR1Items &J)
{
    for (auto i : I.items)
    {
...
void getDFA()
{
    ...
    {
        cout << "LR1Items " << i << " :"<<endl;
        printLR1Items(C->items[i]);
